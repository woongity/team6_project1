...
<script>
	...
	function termSearch(){
		var termSearchForm = document.termSearchForm;
		var fromTime = termSearchForm.fromtime.value;
		var toTime = termSearchForm.totime.value;
	...
	<div class="d-flex justify-content-center">
		<div class="col-8">
			...
			<div class="my-5 d-flex justify-content-center">
				<div class="btn btn-outline-dark col-2 me-1" id="historyBtn"
					onclick="getHistory()">쇼핑 내역</div>
				<div class="btn btn-dark col-2 me-1" id="profileBtn"
					onclick="getProfile()">내 정보</div>
				<div class="btn btn-dark col-2" id="couponBtn" onclick="getCoupon()">쿠폰
					목록</div>
			</div>
			...
			<div id="historyPage" style="display: block;">
				...
				<div class="d-flex justify-content-center my-3">
					<form name="termSearchForm" action="/mypage" method="get">
						<input type="date" id="fromtime" name="fromtime" value="${fromtime}"> 
						<span class="mx-2">~</span> 
						<input type="date" id="totime" name="totime" value="${totime}"> 
						<input class="btn btn-outline-dark btn-sm" type="button" onclick="termSearch()" value="조회">
						<a class="btn btn-dark btn-sm" href="${pageContext.request.contextPath}/mypage">전체</a>
					</form>
				</div>
				...
				<div class="modal fade" id="termModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							...
							<div class="modal-body fw-bold">검색 시작 기간과 종료 기간을 모두 입력해주세요 :)</div>
				...
				<table class="table table-striped" id="page1"
					style="display: block;">
					...
					<tbody>
						...
						<c:forEach var="item" items="${orderedList}" varStatus="oistatus">
							...
							<tr>
								...
								<td><img src="${item.pimage1}" class="card-img-top"
								...
								<td>
									<div>
										...
										<h6>&nbsp;</h6>
										<h6 class="text-muted">
											Color: <img alt="" src="${item.pcolorimage}"
												style="width: 20px; height: 20px;"> ${item.pcolor} /
											Size: ${item.psize}
										</h6>
									</div>
								</td>
								...
								<td class="text-center align-middle"><c:if
										test="${item.ostatus == 1}">
										<h6>결제완료</h6>
									</c:if> <c:if test="${item.ostatus == 0}">
										<h6>주문취소</h6>
									</c:if>
									<h6>(${item.otime})</h6>
									<form action="order/delete" method="post">
										<!-- 주문취소 확인 Modal -->
										<div class="modal" id="getDeleteModal${oistatus.index}"
											tabindex="-1">
											<div class="modal-dialog">
												<div class="modal-content">
													...
													<div class="d-flex justify-content-center">
														<img alt=""
															src="${pageContext.request.contextPath}/resources/images/error_cat.png"
															class="pop" style="width: 50%;">
													</div>
										...
										<input type="text" id="oid" name="oid" class="form-control"
											value="${item.oid}" style="display: none;">
										<c:if test="${item.ostatus == 1}">
											<input class="btn btn-sm"
												id="deleteModalBtn${oistatus.index}" value="[ 주문취소 ]"
												type="submit">
										</c:if>
							...
							<script>
								// deleteModal
								$('#deleteModalBtn${oistatus.index}')
										.click(
												function(e) {
													e.preventDefault();
													$(
															'#getDeleteModal${oistatus.index}')
															.modal("show");
												});
							</script>
			...
			<div id="profilePage" style="display: none;">
				...
				<div class="d-flex my-3">
					...
					<div class="col-10">
						...
						<h6 style="color: #c3c4c7;">단, 가입 및 주문/결제와 재입고 알림등에 관련된 이메일과
							SMS는 동의여부와 관계없이 발송됩니다.</h6>
						<!-- 수신여부 체크 -->
						<div class="d-flex mt-4">
							<div class="d-flex me-5">
								...
								<div class="form-check me-3">
									<input class="form-check-input" type="radio"
										name="flexRadioDefault1" id="flexRadioDefault2"> <label
										class="form-check-label" for="flexRadioDefault2"> 아니오
									</label>
								</div>
							</div>
							<div class="d-flex">
								...
								<div class="form-check me-3">
									<input class="form-check-input" type="radio"
										name="flexRadioDefault2" id="flexRadioDefault1" checked>
									<label class="form-check-label" for="flexRadioDefault1">
										예 </label>
								</div>
								<div class="form-check">
									<input class="form-check-input" type="radio"
										name="flexRadioDefault2" id="flexRadioDefault2"> <label
										class="form-check-label" for="flexRadioDefault2"> 아니오
									</label>
								</div>
							</div>
			...
			<div class="d-flex justify-content-center mt-5">
				<a href="${pageContext.request.contextPath}/list/view"
					class="btn btn-outline-dark col-3 me-3">쇼핑 계속하기</a> <a
					href="${pageContext.request.contextPath}/cart/list"
					class="btn btn-dark col-3 me-3">쇼핑백 보기</a>
			</div>
		</div>
	</div>
